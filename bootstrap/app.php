...
$app = Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        ...
        $middleware->alias([
            'demo.guard' => \App\Http\Middleware\DemoModeGuard::class,
            ...
            'plugin.enabled' => \App\Http\Middleware\EnsurePluginEnabled::class,
        ]);
    })
...
// RoutingServiceProvider builds UrlGenerator with $app->rebinding('request', …), which returns
// null if nothing is bound yet. Some boot steps can resolve url() before SetRequestForConsole.
// A minimal CLI request here matches Laravel's own SetRequestForConsole and is overwritten later.
if ($app->runningInConsole() && ! $app->bound('request')) {
    $base = rtrim((string) (getenv('APP_URL') ?: getenv('GETFY_APP_URL') ?: 'http://localhost'), '/');
    if ($base === '') {
        $base = 'http://localhost';
    }
    $app->instance('request', Request::create($base.'/', 'GET'));
}
