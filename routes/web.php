...
Route::get('/favicon.ico', function () {
    return redirect(url('/icons/favicon.png'), 302);
});

// PWA Painel: manifest e service worker
Route::middleware('plugin.enabled:pwa')->group(function () {
...
Route::middleware('plugin.enabled:coproducao')->group(function () {
...
Route::middleware('guest')->group(function () {
    ...
    Route::post('/demo/login/admin', [\App\Http\Controllers\Auth\DemoLoginController::class, 'loginAsAdmin'])
        ->name('demo.login.admin')
        ->middleware('throttle:10,1');
    Route::post('/demo/login/infoprodutor', [\App\Http\Controllers\Auth\DemoLoginController::class, 'loginAsInfoprodutor'])
        ->name('demo.login.infoprodutor')
        ->middleware('throttle:10,1');
...
Route::prefix('plataforma')->name('plataforma.')->group(function () {
    ...
    Route::middleware(['auth', 'platform.admin'])->group(function () {
        ...
        Route::get('/conta', [\App\Http\Controllers\Platform\PlatformProfileController::class, 'index'])->name('account.index');
        Route::put('/conta/email', [\App\Http\Controllers\Platform\PlatformProfileController::class, 'updateEmail'])->name('account.email');
        Route::put('/conta/senha', [\App\Http\Controllers\Platform\PlatformProfileController::class, 'updatePassword'])
...
Route::middleware(['auth', 'admin.tenant', 'seller.panel', 'role:infoprodutor|team|admin', 'team.permission:equipe.manage'])
    ...
    Route::post('/coproducao/convite/{token}/aceitar', [\App\Http\Controllers\CoproductionInviteController::class, 'accept'])
        ...
        ->middleware(['throttle:20,1', 'plugin.enabled:coproducao'])
        ->where('token', '[A-Za-z0-9]{32,64}');
    Route::post('/painel/idioma', [\App\Http\Controllers\PanelLanguageController::class, 'switch'])->name('panel.language.switch');
    Route::post('/painel/push-subscribe', [\App\Http\Controllers\PanelPwaController::class, 'pushSubscribe'])->name('panel.pwa.push-subscribe')->middleware(['throttle:10,1', 'plugin.enabled:pwa']);
    ...
    Route::middleware('team.permission:produtos.view')->group(function () {
        Route::middleware('plugin.enabled:afiliados')->group(function () {
        ...
        Route::get('/produtos/coproducao', [\App\Http\Controllers\ProdutosController::class, 'coproductionIndex'])->name('produtos.coproducao')->middleware('plugin.enabled:coproducao');
        ...
        Route::post('/produtos/{produto}/coproducers', [\App\Http\Controllers\ProductCoproductionController::class, 'store'])->name('produtos.coproducers.store')->middleware(['throttle:30,1', 'plugin.enabled:coproducao']);
        Route::delete('/produtos/{produto}/coproducers/{coproducer}', [\App\Http\Controllers\ProductCoproductionController::class, 'destroy'])->name('produtos.coproducers.destroy')->middleware('plugin.enabled:coproducao');
        Route::middleware('plugin.enabled:afiliados')->group(function () {
